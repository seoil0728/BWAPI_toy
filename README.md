...
### RPA 시스템 기능 추가
 첫 번째는 파일을 옮기는 것을 넘어 직접 ChaosLauncher를 실행하고, 만약 exe, jar 같은 특수한 실행 형태를 가진 파일 역시 실행할 수 있도록   
 pywinauto 라이브러리를 사용하여 제공할 예정   
 두 번째는 봇 이름 검색 시 중복으로 걸릴 수 있는 폴더들이 있는데 이를 리스트에 담아 표시하고 유저가 선택할 수 있도록 한다.   
 입력을 받으면 전체적으로 서치한 후 조건에 만족하는 폴더명을 리스트에 담아 출력하여 선택할 수 있도록 하는 방법을 구상중.   
...
+ RPA_dowanload_bot.py
  - SSCAIT 사이트에 접속하여, 최신 버전의 파일들을 자동으로 다운로드 하는 파일입니다.
  - 업데이트 로그를 남겨, 마지막 실행 이후 최신 버전이 추가되었을 경우에만 다운로드합니다.
  - 아직 기능 구현이 되지 않은 파일입니다.


+ RPArunchaos.py
  - chaoslauncher와 특정 봇 실행 파일을 자동화하기 위한 파일입니다.
  - RPAbwapi.py 파일에 연동시켜 좀 더 편한 자동화 기능을 제공할 예정입니다.
  - 아직 기능 구현이 되지 않은 파일입니다.
